...
We have used the \Mlr process to estimate the impact of the track quality on the mass resolution. The square root of the center of mass energy in the \Mlr process, is fixed for a given beam energy, and is equal:
\begin{equation}
 \dstl M_{ee}^{\mathrm{c.m.}} = \sqrt{2\cdot m_{e}\cdot E_{b}}
\end{equation}
This is will be equal to the invariant mass of final state electrons in the \Mlr process.
...
$\sigma_{\mathrm{stat}}$, and $\dstl \sigma_{m}$ for $\delta m$, FOM can be expressed as
\begin{equation}
