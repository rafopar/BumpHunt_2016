...
One can notice the MC is time resolution is significantly narrow. Because of this instead of a
fix cut, for all data samples (Data, MC), we chose different cuts for Data and MC respectively, in other words these 99\% cut limits for data is obtained using events data events, while for MC 99\% 
limits limits were obtained from MC sample. Broadening at very low momenta is because of the way, how cuts are defined. As described in \ref{sec:2Dcuts}, when the number of events in the given momentum
bin gets smaller and smaller (less than 100, 60, 30, 10) cuts gets looser and looser, and that
caused very few event at lower momenta to make cuts so large, however in the final data sample, practically there are no such events. No difference is observed for positrons, and we have didn't put these plots in this document (although can be found in \cite{evSelectionPlots}).
...
%  \subfloat[]{\grinp[width=0.25\tw]{Figs/dX_epBotNoL6_Data_AllBut.pdf}}
 ...
 corresponds to tracks that don't don't have 3D hit in L6.
...
%  \subfloat[]{\grinp[width=0.25\tw]{Figs/dX_epBotNoL6_Tri_AllBut.pdf}}
 ...
 corresponds to tracks that don't don't have 3D hit in L6. Note: Each histogram is
...
\vskip 1 cm
{\color{Red} Add descriptions to the following pictures}
